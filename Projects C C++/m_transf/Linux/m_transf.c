...
TEST (Filtering_functio_test, PositiveNos) {
	int size = 10;
	int i = 0, j =0;
	complexm* mtx1 = cmt_alloc(size);
	complexm* mtx2 = cmt_alloc(size);
	complexm* mtx3 = cmt_alloc(size);
	complexm* out = cmt_alloc(size);
	...
	for(i = 0; i < size;i++)
	{
		for(j = 0; j < size; j++)
		{
			EXPECT_EQ(out[i+j*size], mtx2[i+j*size]);
		}
	}
...
int main(int argc, char* argv[])
{
    ...
    mtx1d[7] = 0;
    mtx1d[8] = 1;
    double_mtx_to_complexm(mtx1d,mtx1,size);
*/
    /*

    init_reim(&mtx1[0], 1, 0);
    init_reim(&mtx1[1], 0, 0);
    init_reim(&mtx1[2], 0, 0);
    init_reim(&mtx1[3], 0, 0);
    init_reim(&mtx1[4], 1, 0);
    init_reim(&mtx1[5], 0, 0);
    init_reim(&mtx1[6], 0, 0);
    init_reim(&mtx1[7], 0, 0);
    init_reim(&mtx1[8], 1, 0);*/
    //for (int i = 0; i < 9; i++)
    //    mtx1[i].rfi = false;
   /* ret_err errors = mtx_invers(mtx1, mtx2, size, buff);
    if (errors != SUCCESS) printf("ERROR %d", errors);
    int i = 0;
